...
// Renders the dashboard (identical design to board.html)
 ...
 * 'pending' is treated as 'todo' (HU-08 db.json uses 'pending' as initial status).
 */
...
export async function renderBoard(container) {
  ...
  // Render shell (sidebar + topbar + empty columns)
...
function renderColumns(tasks, users) {
  ...
  COLUMNS.forEach(col => {
    ...
    const counter = document.getElementById(`counter-${col.key.replaceAll(' ', '-')}`);
    if (!container) return;
  ...
  // Re-attach card edit listeners after re-render
  attachCardListeners();
}

/** Attach listeners to edit buttons on task cards */
...
/** Returns the full board HTML (identical layout to board.html) */
